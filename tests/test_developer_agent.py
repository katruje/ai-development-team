...
class TestDeveloperAgent(unittest.IsolatedAsyncioTestCase):
    ...
    async def asyncSetUp(self):
        ...
        # _process_message is no longer patched to allow testing actual agent logic
    ...
    async def test_help_message(self):
        ...
        expected_help_message = self.agent._get_help_message() # Get actual help message
        ...
        self.assertIn("- analyze <requirements>: Analyze software requirements", response.content)
        self.assertIn("- generate <task>: Generate code for the given task", response.content)
    ...
    async def test_unknown_command(self):
        ...
        response = await self.agent._process_message(message, self.context)
        self.assertEqual(response.content, "Unknown command. Type 'help' for available commands.")
    ...
    async def test_code_generation(self):
        ...
        self.assertIn("Generated code for task: a python function called factorial", response.content)
        self.assertIn("def _apythonfunction():", response.content) # Check for part of the generated code
        
        message = AgentMessage(
            role=AgentRole.DEVELOPER,
            content="generate something else"
        )
        response = await self.agent._process_message(message, self.context)
    ...
    async def test_file_creation(self):
        ...
        message = AgentMessage(
            role=AgentRole.DEVELOPER,
            content=f"create file {empty_filename}"
        )
        ...
        response = await self.agent._process_message(message, self.context)
        self.assertEqual(response.content, "Error: File path not specified for 'create file' command.")
    ...
    #     self.assertIsNone(self.agent.get_code_template("non_existent"))

    # def test_log_file_operation(self):
    #     """Test logging file operations."""
    #     with patch('agent_core.agents.developer.agent.logger') as mock_logger:
    #         # This test needs to be rewritten as log_file_operation was removed.
    #         # Direct logging calls would be tested by checking mock_logger calls.
    #         pass
...
class TestDeveloperAgentIntegration(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_file_operations(self):
        ...
        message = AgentMessage(
            role=AgentRole.DEVELOPER,
            content=f"create file subdir/test_file.txt with content: {test_content}"
        )
