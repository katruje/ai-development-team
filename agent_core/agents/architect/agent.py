...
class ProjectAnalyzer:
            ...
            with open(file_path, 'rb') as f:
                ...
                if 'project' in data and 'dependencies' in data['project']:
                    for dep in data['project']['dependencies']:
                        pkg = dep.split('>=')[0].split('>')[0].split('<=')[0].split('<')[0].split('~=')[0].split('!=')[0]
    ...
    def _get_project_structure(self) -> List[Dict[str, Any]]:
        ...
        for path in sorted(self.project_root.iterdir()):
            if path.name.startswith(('.', '__pycache__', 'venv', '.venv', 'env', '.env')):
                continue
...
class ArchitectAgent(Agent):
    ...
    async def _process_message(self, message: AgentMessage, context: AgentContext) -> AgentMessage:
        ...
        if any(term in content for term in ["project structure", "show project", "list files"]):
        ...
        return AgentMessage(
            role=self.role,
            content=("I'm the Architect agent. I can help analyze your project structure, "
                    "dependencies, and provide architectural guidance. Try asking me to 'analyze the project' "
                    "or 'show project structure'.")
        )
    ...
    async def _analyze_project(self, context: AgentContext) -> AgentMessage:
        ...
        with console.capture() as capture:
            ...
            table.add_row("Imports", ", ".join(analysis['imports'][:5]) + 
