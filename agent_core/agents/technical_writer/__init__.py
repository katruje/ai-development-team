...
class TechnicalWriterAgent(Agent):
    ...
    Attributes:
        doc_formats (List[str]): Supported documentation formats (e.g., 'markdown', 'rst')
        doc_style (str): Documentation style guide to follow (e.g., 'google', 'numpy')
        include_examples (bool): Whether to include code examples in documentation
    """
    ...
    async def generate_documentation(
        ...
        This method analyzes the target code and generates comprehensive documentation
        including API references, usage examples, and module/package documentation.
        
        Args:
            target_path: Path to the target file or package to document
            output_format: Output format for the documentation (e.g., 'markdown', 'html')
        ...
        """
        try:
            logger.info("Generating %s documentation for %s", output_format, target_path)
            ...
            return AgentMessage(
                role=self.role,
                content=f"Documentation generated successfully in {output_dir or 'docs/generated'}",
                metadata={
            ...
            return self._create_error_response(f"Failed to generate documentation: {str(e)}")
    ...
    async def validate_documentation(self, target_path: str) -> AgentMessage:
        ...
        try:
            ...
            return self._create_error_response(f"Documentation validation failed: {str(e)}")
