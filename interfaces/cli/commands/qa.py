...
def run_tests(
    test_path: str = typer.Argument(
        "tests/",
        help="Path to test file or directory to run. Can be a directory containing test files "
            "or a specific test file. Defaults to 'tests/'."
    ),
    coverage: bool = typer.Option(
        False,
        "--coverage",
        "-c",
        help="Enable test coverage reporting. Generates a coverage report showing which "
             "parts of the code are covered by tests."
    ),
    threshold: Optional[float] = typer.Option(
        None,
        "--threshold",
        "-t",
        help="Set minimum test coverage threshold as a percentage (0-100). If the actual "
             "coverage is below this threshold, the command will fail. Requires --coverage.",
        min=0,
        max=100
    )
):
    ...
    This command runs the specified tests and displays a formatted report of the results,
    including pass/fail counts and optional coverage information. The command will exit
    with a non-zero status code if any tests fail or if coverage is below the specified
    threshold.
    ...
    if response.metadata.get("command") == "test_results":
        ...
        table.add_row("Failed", f"[red]{results['failed']}" if results['failed'] > 0 else "0")
        table.add_row("Errors", f"[red]{results['errors']}" if results['errors'] > 0 else "0")
        ...
        if threshold and ("coverage" not in results or results["coverage"] is None):
            console.print("[yellow]Warning: Coverage threshold specified but no coverage data available[/]")
...
def generate_tests(
    ...
    test_type: str = typer.Option(
        ...
        "--type",
        "-t",
        help="Type of tests to generate. Supported values: 'unit' (default), 'integration'. "
             "Unit tests test individual components in isolation, while integration tests verify "
    ...
    output_dir: Optional[Path] = typer.Option(
        ...
        "--output",
        "-o",
        help="Output directory for generated test files. If not specified, tests will be "
    ...
    test cases based on the implementation. The generated tests include common test cases
    and edge cases to ensure good code coverage.
    ...
    Examples:
        ...
        ai-dev-team qa generate-tests myproject/ --type integration --output tests/integration
